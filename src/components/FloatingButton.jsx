  ...
  return (
    <div className="fixed bottom-5 right-5 z-[70] flex flex-col items-end gap-3">
      {actions.map((a, i) => (
        ...
        >
          <span className="px-3 py-1.5 text-xs font-medium text-white bg-gray-900 dark:bg-white dark:text-gray-900 rounded-lg shadow-lg">
            {a.label}
          </span>
          <div className={`w-11 h-11 rounded-full bg-gradient-to-br ${a.color} flex items-center justify-center text-white shadow-lg hover:scale-110 transition-transform`}>
            {a.icon}
          </div>
        </a>
      ))}
      ...
      >
        {open ? (
          ...
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeWidth={2} d="M20 12H4m8-8v16" />
          </svg>
        )}
      </button>
    </div>
