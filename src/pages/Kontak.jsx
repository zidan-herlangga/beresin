...
const contacts = [
  ...
  {
    label: "Email",
    value: "halo@beresin.com",
    href: "mailto:halo@beresin.com",
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="space-y-4">
              {contacts.map((c) => (
                ...
                >
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${c.gradient} flex items-center justify-center text-white group-hover:scale-110 transition-transform shrink-0`}>
                    {c.icon}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-white">{c.label}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{c.value}</p>
                  </div>
                  <svg className="w-4 h-4 ml-auto text-gray-300 dark:text-gray-600 group-hover:text-indigo-400 transition-colors shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form onSubmit={handleSubmit} className={`p-6 rounded-2xl bg-gray-50 dark:bg-gray-800/20 border border-gray-100 dark:border-gray-700/30 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
              ...
              <div className="space-y-3">
                ...
                <button
                  type="submit"
                  className="w-full py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-sm font-semibold rounded-xl hover:shadow-lg hover:shadow-indigo-500/20 transition-all"
                >
                  Kirim via WhatsApp
                </button>
              </div>
