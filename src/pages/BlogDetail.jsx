...
<p>Dengan perencanaan yang matang, skripsi 3 bulan bukan mimpi!</p>`,
  ...
  useJsonLd({
    ...
    publisher: { "@type": "Organization", name: "Beresin Tugas", logo: { "@type": "ImageObject", url: `${siteUrl}/logo.svg` } },
    mainEntityOfPage: { "@type": "WebPage", "@id": pageUrl },
  });

  const stripHtml = (html) => html?.replace(/<[^>]*>/g, "") || "";
  const readingTime = Math.max(1, Math.ceil((stripHtml(a.content).split(/\s+/).length || 0) / 200));
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-gray-900 pt-28 pb-16">
      ...
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav className="flex items-center gap-2 text-sm text-gray-400 mb-6">
          ...
          <svg className="w-3 h-3 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
          <span className="text-gray-500 truncate max-w-[180px]">{a.title}</span>
        </nav>
        ...
        {a.excerpt && (
          <p className="text-base text-gray-500 dark:text-gray-400 mb-6 leading-relaxed">{a.excerpt}</p>
        )}
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 pb-6 border-b border-gray-200 dark:border-gray-700/50 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-white">Beresin Tugas</p>
        ...
        <div className="mt-12 pt-8 border-t border-gray-200 dark:border-gray-700/50">
          <div className="flex items-center justify-between flex-wrap gap-4 mb-8">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Bagikan artikel ini:</p>
            <ShareButtons url={pageUrl} title={a.title} />
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-gray-800 dark:to-gray-850 border border-indigo-100 dark:border-gray-700/50">
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-white">Butuh bantuan tugas?</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Tim Beresin siap membantu tugas kuliah dan skripsimu.</p>
            </div>
            <div className="flex gap-3">
              <a href="https://wa.me/6285157066514" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 bg-green-600 text-white text-sm font-semibold rounded-xl hover:bg-green-500 transition-all active:scale-[0.97] shadow-lg shadow-green-600/20">
              ...
              <Link to="/" className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-sm font-semibold rounded-xl hover:from-indigo-500 hover:to-purple-500 transition-all active:scale-[0.97] shadow-lg shadow-indigo-500/20">
