...
function Svg({ d, className }) {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
    >
      <path strokeLinecap="round" strokeWidth={1.5} d={d} />
    </svg>
  );
}
...
function Toast({ message, type, onClose }) {
  ...
  return (
    <div className="fixed top-6 right-6 z-50 animate-slideDown">
      <div
        className={`flex items-center gap-3 px-5 py-3.5 rounded-xl border shadow-2xl backdrop-blur-xl text-sm min-w-[320px] ${
          type === 'success'
            ? 'bg-emerald-900/85 border-emerald-700/40 text-emerald-200'
            : 'bg-red-900/85 border-red-700/40 text-red-200'
        }`}
      >
        <span
...
function ConfirmModal({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative bg-gray-900 border border-white/[0.08] rounded-2xl p-6 max-w-md w-full shadow-2xl animate-scaleIn"
        onClick={(e) => e.stopPropagation()}
      >
        <div
          className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${danger ? 'bg-red-500/15 text-red-400' : 'bg-indigo-500/15 text-indigo-400'}`}
        >
        ...
        <div className="flex gap-3 justify-end">
          <button
            onClick={onCancel}
            className="px-5 py-2.5 bg-white/[0.05] hover:bg-white/[0.1] text-gray-300 rounded-xl text-sm font-medium transition-all"
          >
            Batal
          </button>
          <button
            onClick={onConfirm}
            className={`px-6 py-2.5 rounded-xl text-sm font-semibold text-white transition-all active:scale-[0.97] ${danger ? 'bg-red-600 hover:bg-red-500' : 'bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500'}`}
          >
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-6 relative overflow-hidden">
      <div className="absolute inset-0">
        ...
        <div
          className="absolute bottom-1/3 -right-40 w-[500px] h-[500px] bg-purple-500/10 rounded-full blur-3xl animate-pulse"
        ...
        <div className="relative bg-gray-900/90 backdrop-blur-xl rounded-2xl p-8 border border-white/5">
          ...
          <div className="relative mb-4">
            <input
              ...
              className="w-full px-4 py-3.5 pr-12 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-600 text-sm focus:outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 transition-all"
              autoFocus
            />
            <button
              type="button"
          ...
          >
            ...
            {loading && (
              <span className="absolute inset-0 flex items-center justify-center">
                <Svg d={icons.spinner} className="w-5 h-5 animate-spin" />
              </span>
            )}
          </button>
        </div>
...
function Field({
  ...
  const base =
    'w-full px-4 py-3 bg-white/[0.03] border border-white/[0.08] rounded-xl text-white placeholder-gray-600 text-sm focus:outline-none focus:border-indigo-500/40 focus:ring-2 focus:ring-indigo-500/15 transition-all duration-200';
  return (
      ...
      <div className="relative">
        ...
        {type === 'textarea' ? (
          <textarea
            ...
            className={base + ' resize-none pl-4'}
          />
        ) : (
          <input
            id={id}
...
function ItemCard({
  ...
  const handleDrop = (e) => {
    ...
    if (from !== index && onMoveUp) onMoveUp(from, index);
  };
  ...
  return (
    <div
      ...
      className={`group relative bg-white/[0.02] rounded-2xl border transition-all duration-200 ${
        dragOver
          ? 'border-indigo-500/50 bg-indigo-500/8 ring-2 ring-indigo-500/25 shadow-lg shadow-indigo-500/5'
          : 'border-white/[0.06] hover:border-white/[0.12] hover:bg-white/[0.04] hover:shadow-lg'
      } p-5 mb-3`}
    >
      {/* Top bar */}
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/[0.04]">
        <div className="flex items-center gap-2.5">
          ...
          <span className="flex items-center justify-center w-6 h-6 rounded-lg bg-white/[0.04] text-[11px] font-mono font-semibold text-gray-500">
            {index + 1}
          </span>
          {onMoveUp && !isFirst && (
            <button
              onClick={() => onMoveUp(index, index - 1)}
              className="text-gray-600 hover:text-gray-200 p-1 rounded-lg hover:bg-white/[0.06] transition-all"
            >
        ...
        <div className="flex items-center gap-1">
          {canRemove && (
            <button
              onClick={onRemove}
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-[11px] font-medium text-red-400/60 hover:text-red-300 hover:bg-red-900/15 rounded-lg transition-all opacity-0 group-hover:opacity-100"
            >
    ...
    <div className="mb-6">
      <div className="flex items-center justify-between mb-4 px-1">
        <div className="flex items-center gap-2.5">
          ...
          {items.length > 0 && (
            <span className="text-[11px] text-gray-600 bg-white/[0.04] px-2 py-0.5 rounded-full font-mono">
              {items.length}
            </span>
          )}
        </div>
        <button
          onClick={addItem}
          className="flex items-center gap-1.5 text-xs px-4 py-2 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 font-medium rounded-xl border border-indigo-500/20 hover:border-indigo-500/30 transition-all active:scale-95 shadow-sm"
        >
      ...
      {items.length === 0 ? (
        <div className="text-center py-12 text-gray-500 text-sm border border-dashed border-white/[0.06] rounded-2xl bg-white/[0.01]">
          ...
          <button
            onClick={addItem}
            className="mt-3 text-xs text-indigo-400 hover:text-indigo-300 bg-indigo-600/10 hover:bg-indigo-600/20 px-3 py-1.5 rounded-lg transition-all"
          >
            + Tambah Sekarang
          </button>
        </div>
              ...
              {fields.map((f) => (
                ...
                >
                  {f.type === 'image' ? (
                    ...
                    <>
                      <label className="block text-[11px] text-gray-500 mb-1 font-semibold tracking-wider">
                        {f.label}
                      </label>
                      {f.type === 'textarea' ? (
                        <textarea
                          value={item[f.key] || ''}
                          onChange={(e) =>
                            updateItem(idx, f.key, e.target.value)
                          }
                          rows={f.rows || 2}
                          placeholder={f.placeholder}
                          className="w-full px-3 py-2.5 bg-black/30 border border-white/[0.06] rounded-lg text-white placeholder-gray-700 text-xs focus:outline-none focus:border-indigo-500/40 focus:ring-1 focus:ring-indigo-500/10 transition-all resize-none"
                        />
                      ) : f.type === 'boolean' ? (
                        <select
                          value={String(item[f.key] ?? false)}
                          onChange={(e) =>
                            updateItem(idx, f.key, e.target.value === 'true')
                          }
                          className="w-full px-3 py-2.5 bg-black/30 border border-white/[0.06] rounded-lg text-white text-xs focus:outline-none focus:border-indigo-500/40 transition-all"
                        >
                        ...
                        <input
                          ...
                          placeholder={f.placeholder}
                          className="w-full px-3 py-2.5 bg-black/30 border border-white/[0.06] rounded-lg text-white placeholder-gray-700 text-xs focus:outline-none focus:border-indigo-500/40 transition-all"
                        />
                      )}
                    </>
                  )}
...
function NestedArrayEditor({ label, items, onUpdate, config }) {
  const addItem = () => {
    const e = { title: '', tag: '', items: [] };
    if (config.subItems) e[config.subItems] = [];
    if (config.services) e.services = [];
    onUpdate([...items, e]);
  };
  const removeItem = (idx) => {
    if (items.length <= 1) return;
  ...
  const removeSub = (idx, k, si) => {
    const c = [...items];
    c[idx][k] = c[idx][k].filter((_, i) => i !== si);
    onUpdate(c);
  };
  const updateSub = (idx, k, si, v) => {
    const c = [...items];
    c[idx][k][si] = v;
    onUpdate(c);
  };
  ...
  const renderSub = (idx, k, label, placeholder) => (
    <div className="mt-4 pt-4 border-t border-white/[0.04]">
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          onClick={() => addSub(idx, k)}
          className="flex items-center gap-1 text-xs text-indigo-400/70 hover:text-indigo-300 bg-indigo-600/10 hover:bg-indigo-600/20 px-2.5 py-1 rounded-lg transition-all"
        >
          <Svg d={icons.plus} className="w-3 h-3" /> Tambah
        </button>
      </div>
      {(items[idx][k] || []).length === 0 ? (
        <p className="text-xs text-gray-600 italic bg-white/[0.02] rounded-lg px-3 py-4 text-center">
          Belum ada item
        </p>
      ) : (
        (items[idx][k] || []).map((si, siIdx) => (
          <div key={siIdx} className="flex gap-2 mb-2">
            <input
              value={si}
              onChange={(e) => updateSub(idx, k, siIdx, e.target.value)}
              placeholder={placeholder || 'Item'}
              className="flex-1 px-3 py-2 bg-black/30 border border-white/[0.06] rounded-lg text-white placeholder-gray-700 text-xs focus:outline-none focus:border-indigo-500/40 transition-all"
            />
            <button
              onClick={() => removeSub(idx, k, siIdx)}
              className="text-red-400/50 hover:text-red-300 px-2.5 rounded-lg hover:bg-red-900/15 transition-all"
            >
          ...
          >
            ...
            {config.subItems &&
              renderSub(
                ...
                config.subItems,
                config.subLabel,
                config.subPlaceholder,
              )}
            {config.services &&
              renderSub(
                idx,
                'services',
                config.subLabel || 'Services',
                'Nama layanan',
              )}
          </ItemCard>
        ))
      ...
      {sections.map((s) => (
        ...
        >
          {sectionIcons[s.id] && (
            <Svg
              d={sectionIcons[s.id]}
              className={`w-3.5 h-3.5 ${active === s.id ? 'text-indigo-400' : 'text-gray-500 group-hover:text-gray-300'}`}
            />
          )}
          {s.label}
          {active === s.id && (
...
function DashboardOverview({ onNavigate }) {
  ...
  const cards = useMemo(() => {
    ...
    return [
      {
        label: 'Total Konten',
        value: Object.values(stats).reduce((s, d) => s + countItems(d), 0),
        sub: 'Semua halaman',
      ...
      {
        label: 'Layanan',
        value: stats.layanan?.categories?.length || 0,
        sub: 'Kategori jasa',
    ...
    return [
      {
        ...
        items: [
          { name: 'Hero', count: 1 },
          { name: 'Layanan', count: b.layanan?.length || 0 },
          { name: 'Harga', count: b.harga?.length || 0 },
          { name: 'Cara Order', count: b.caraOrder?.length || 0 },
          { name: 'Tim', count: b.tim?.length || 0 },
          { name: 'Portofolio', count: b.portofolio?.length || 0 },
          { name: 'Garansi', count: b.garansi?.length || 0 },
          { name: 'Testimoni', count: b.testimoni?.length || 0 },
          { name: 'FAQ', count: b.faq?.length || 0 },
          { name: 'Blog', count: b.blog?.length || 0 },
        ],
      },
      {
        id: 'layanan',
        ...
        items: [
          { name: 'Kategori', count: stats.layanan?.categories?.length || 0 },
          {
            name: 'Total Servis',
            count:
              stats.layanan?.categories?.reduce(
                (s, c) => s + (c.services?.length || 0),
                0,
              ) || 0,
          },
        ],
      ...
      <div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="animate-pulse bg-white/[0.02] rounded-2xl border border-white/[0.06] p-5"
            >
    ...
    <div className="space-y-8">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600/10 via-purple-600/5 to-transparent border border-indigo-500/10 p-6 md:p-8">
        ...
        <div className="relative">
          ...
          <p className="text-sm text-gray-500 mt-3 max-w-xl">
            Kelola konten website dari sini. Pantau statistik, edit halaman, dan
            perbarui informasi dengan mudah.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((c, i) => (
          <div
            key={i}
            className="group relative bg-white/[0.02] rounded-2xl border border-white/[0.06] p-5 hover:border-white/[0.12] hover:bg-white/[0.04] transition-all duration-300 active:scale-[0.98]"
          >
            ...
            <div
              className={`relative w-11 h-11 rounded-xl bg-gradient-to-br ${c.color} flex items-center justify-center mb-3 shadow-lg ${c.glow} group-hover:scale-110 transition-transform duration-300`}
            >
              <Svg d={c.icon} className="w-5 h-5 text-white" />
            </div>
            <p className="text-2xl md:text-3xl font-bold text-white group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-gray-300 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
              {c.value}
            </p>
            ...
            return (
              ...
              >
                ...
                <div className="relative space-y-2">
                  {s.items.map((it) => (
                    ...
                    >
                      ...
                      <span className="text-gray-600 group-hover:text-gray-300 font-mono">
                        {it.count}
                      </span>
                    </div>
                  ))}
                ...
                <div className="relative mt-3 pt-3 border-t border-white/[0.04] flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-indigo-500/60 group-hover:text-indigo-400 group-hover:translate-x-1 transition-all">
                    Kelola &rarr;
                  </span>
                </div>
              </button>
...
const caraOrderSections = [
  { id: 'langkah', label: 'Langkah' },
  { id: 'faq', label: 'FAQ' },
];
const tentangSections = [
  { id: 'hero', label: 'Hero' },
...
function BerandaEditor({ data, setData, blogMode }) {
  ...
  const setStats = (v) =>
    setData({ ...data, hero: { ...data.hero, stats: v } });
  return (
    <div className="animate-fadeIn">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
          <div className="sm:col-span-2">
            <Field
              ...
              placeholder="cth: Tugas Menumpuk?"
            />
          </div>
          <div className="sm:col-span-2">
            <Field
          ...
          <div className="sm:col-span-2">
            <ArrayEditor
              ...
              fields={[
                ...
                { key: 'label', label: 'Label', placeholder: 'Tugas Selesai' },
              ]}
            />
          </div>
        </div>
      ...
      {section === 'layanan' && (
        <NestedArrayEditor
          ...
          config={{
            subItems: 'items',
            subLabel: 'Daftar Layanan',
            subPlaceholder: 'Nama layanan',
          }}
        />
      )}
      {section === 'harga' && (
          ...
          fields={[
            {
              key: 'name',
              label: 'Nama Paket',
              placeholder: 'Pelajar / Mahasiswa / PRO',
            },
            {
              key: 'price',
              label: 'Harga',
              placeholder: '25rb / 250rb / 500rb',
            },
            { key: 'period', label: 'Period', placeholder: '/tugas' },
            {
              key: 'desc',
              label: 'Deskripsi',
      ...
      {section === 'cara-order' && (
        <ArrayEditor
          ...
          itemLabel="Langkah"
          fields={[
            { key: 'number', label: 'Nomor', placeholder: '01' },
            { key: 'title', label: 'Judul', placeholder: 'Konsultasi' },
      ...
      {section === 'tim' && (
        <ArrayEditor
          ...
          itemLabel="Anggota"
          fields={[
            { key: 'name', label: 'Nama', placeholder: 'Zidan Herlangga' },
            { key: 'role', label: 'Role', placeholder: 'Founder' },
            { key: 'initials', label: 'Inisial', placeholder: 'ZH' },
            {
              key: 'gradient',
              label: 'Gradient',
      ...
      {section === 'portofolio' && (
        <ArrayEditor
          ...
          itemLabel="Item"
          fields={[
            { key: 'title', label: 'Judul', placeholder: 'Nama project' },
            { key: 'category', label: 'Kategori', placeholder: 'Programming' },
            {
              key: 'desc',
              label: 'Deskripsi',
      ...
      {section === 'garansi' && (
        <ArrayEditor
          ...
          itemLabel="Garansi"
          fields={[
            { key: 'title', label: 'Judul', placeholder: 'Tepat Waktu' },
            { key: 'desc', label: 'Deskripsi', type: 'textarea', rows: 2 },
          ]}
        />
      )}
      {section === 'testimoni' && (
        <ArrayEditor
          ...
          itemLabel="Testimoni"
          fields={[
            { key: 'name', label: 'Nama', placeholder: 'Rina' },
            { key: 'role', label: 'Role', placeholder: 'Mahasiswi UI' },
            { key: 'text', label: 'Testimoni', type: 'textarea', rows: 2 },
            { key: 'initials', label: 'Inisial', placeholder: 'RN' },
            {
              key: 'gradient',
              label: 'Gradient',
      ...
      {section === 'faq' && (
        <ArrayEditor
          ...
          itemLabel="FAQ"
          fields={[
            { key: 'q', label: 'Pertanyaan', placeholder: 'Apa itu Beresin?' },
      ...
      {section === 'blog' && (
        <ArrayEditor
          ...
          fields={[
            ...
            { key: 'tag', label: 'Tag', placeholder: 'Tips' },
            { key: 'date', label: 'Tanggal', placeholder: '15 Mar 2026' },
            { key: 'image', label: 'Gambar', type: 'image' },
        ...
        <ArrayEditor
          ...
          fields={[
            ...
            { key: 'desc', label: 'Deskripsi', type: 'textarea', rows: 2 },
            {
              key: 'icon',
              label: 'Icon',
              placeholder: 'chat / check / gear / done',
            },
          ]}
        />
      )}
    ...
    <div className="animate-fadeIn">
      ...
      {section === 'hero' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
          <Field
            ...
            placeholder="Kamu punya masalah dengan tugas?"
          />
          <div className="sm:col-span-2">
            <Field
              label="Subtitle"
      ...
      {section === 'cerita' && (
        <div>
          ...
          {(data.story || ['']).map((p, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <textarea
                ...
                rows={2}
                placeholder="Paragraf cerita..."
                className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-600 text-sm focus:outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 transition-all resize-none"
              />
              {i > 0 && (
                <button
                  onClick={() => {
                  ...
                  className="text-red-400/60 hover:text-red-300 px-2"
                >
                  <Svg d={icons.x} className="w-4 h-4" />
                </button>
              )}
          ...
          <button
            onClick={() =>
              setData({ ...data, story: [...(data.story || []), ''] })
            }
            className="flex items-center gap-1.5 text-xs text-indigo-400/70 hover:text-indigo-300 transition mt-2"
          >
    ...
    <div className="animate-fadeIn">
      ...
      {section === 'kontak' && (
        <ArrayEditor
          ...
          itemLabel="Kontak"
          fields={[
            { key: 'label', label: 'Label', placeholder: 'WhatsApp / Email' },
            { key: 'value', label: 'Nilai', placeholder: '0851-5706-6514' },
            { key: 'href', label: 'Link', placeholder: 'https://wa.me/...' },
            { key: 'icon', label: 'Icon', placeholder: 'wa / email / ig' },
          ]}
        />
      )}
      {section === 'social' && (
        <ArrayEditor
          ...
          itemLabel="Social"
          fields={[
            { key: 'name', label: 'Nama Platform', placeholder: 'Instagram' },
            {
              key: 'url',
              label: 'URL',
...
function AdminDashboard({ token, onLogout }) {
  ...
  const showToast = useCallback(
    (msg, type) => setToast({ message: msg, type, key: Date.now() }),
    [],
  );
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 w-64 h-screen bg-gray-900/95 backdrop-blur-xl border-r border-white/[0.04] flex flex-col shrink-0 transition-all duration-300 ease-out ${
      ...
      >
        <div className="relative p-5 border-b border-white/[0.04]">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => setSidebarOpen(false)}
              className="lg:hidden w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:text-white hover:bg-white/[0.06] transition-all -mr-1"
            >
              <Svg d={icons.x} className="w-4 h-4" />
            </button>
          </div>
        ...
        <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
          {pages.map((p) => {
            ...
            return (
              ...
              >
                {isActive && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 rounded-full bg-gradient-to-b from-indigo-400 to-purple-500" />
                )}
                <Svg
                  d={icons[p.id]}
                ...
                {dirty && isActive && p.id !== 'dashboard' && (
                  <span className="ml-auto w-2 h-2 rounded-full bg-amber-400 animate-pulse shadow-lg shadow-amber-400/20" />
                )}
              </button>
            );
          })}
      ...
      <main className="flex-1 flex flex-col min-h-screen overflow-y-auto">
        <div className="sticky top-0 z-30 lg:hidden bg-gray-950/90 backdrop-blur-xl border-b border-white/[0.04] px-4 py-2.5 flex items-center justify-between">
          <button
            ...
            className="w-10 h-10 flex items-center justify-center rounded-xl text-gray-400 hover:text-white hover:bg-white/[0.06] -ml-1.5 transition-all active:scale-90"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            {dirty && (
              <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse shadow-lg shadow-amber-400/20" />
            )}
          </div>
        </div>
                  ...
                  <div className="sticky bottom-4 z-20 mt-6 -mx-1 px-1">
                    <div className="bg-gray-900/95 backdrop-blur-xl border border-white/[0.06] rounded-2xl px-5 py-3.5 flex items-center gap-3 shadow-2xl shadow-black/40">
                      ...
                      <span className="ml-auto text-xs text-gray-600 bg-white/5 px-3 py-1.5 rounded-full hidden sm:inline-flex items-center gap-1.5">
                        <kbd className="px-1.5 py-0.5 bg-white/[0.06] rounded text-[10px] font-mono">
                          Ctrl
                        </kbd>
...
export default function Admin() {
  const [token, setToken] = useState(
    localStorage.getItem('beresin-admin-token'),
  );
